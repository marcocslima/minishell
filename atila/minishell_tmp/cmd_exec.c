...
/*   Updated: 2022/10/09 02:20:59 by acosta-a         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	cmd_check_2_1(t_data **data, t_cursors *crs)
{
	...
	while (crs->flagpipe == 8 && crs->i2 <= crs->k2 && (*data)->cmds[crs->i2] && (*data)->cmds[crs->i2][crs->j2] && !ft_memcmp((*data)->cmds[crs->i2][crs->j2], "<", 2))
		ft_input(data, crs);
...
void	builtin_execute_input(t_data **data, int i, int flag, t_cursors *crs)
{
	char	*cmd1;
	int		j;
	...
	if (!ft_memcmp((*data)->cmds[i][0], "echo", 5))
	{
		...
		builtin_execute_2_input(data, i, flag, cmd1, crs);
}

void	builtin_execute_2_input(t_data **data, int i, int flag, char *cmd1, t_cursors *crs)
{
	if (!ft_memcmp((*data)->cmds[i][0], "./", 2) ||
			!ft_memcmp((*data)->cmds[i][0], "../", 3) ||
				!ft_memcmp((*data)->cmds[i][0], "/", 1))
